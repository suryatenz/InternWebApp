...
const SpecificationsValidation = () => {
    ...
    const handleValidationClick = (attributeName, validation) => {
              ...
              <li key={index} className='mt-2'>
                <div className='flex items-center'>
                  ...
                  <div className="w-1/4 flex">
                    {["cantvalidate", "validate", "cantsay"].map((status) => (
                      <div
                        key={status}
                        className={`px-4 py-2 mr-2 cursor-pointer ${validationStatus[attribute.name] === status ? 'bg-orange-500' : 'bg-neutral-800'} text-white`}
                        style={{
                          borderRadius: '4px',
                          border: '1px solid orange'
                        }}
                        onClick={() => handleValidationClick(attribute.name, status)}
                      >
                        {status}
                      </div>
                    ))}
...
export default SpecificationsValidation
